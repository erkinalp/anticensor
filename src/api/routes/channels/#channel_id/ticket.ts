...
router.patch(
	...
	async (req: Request, res: Response) => {
		...
		if (!ticket.parent_id) return res.status(400).send({ message: "Not a thread" });
		if (ticket.type !== ChannelType.GUILD_PRIVATE_THREAD && ticket.type !== ChannelType.GUILD_PUBLIC_THREAD) return res.status(400).send({ message: "Not a ticket thread" });
		...
		if (payload.public !== undefined) {
			...
			if (payload.public) {
				if (ticket.type === ChannelType.GUILD_PRIVATE_THREAD) ticket.type = ChannelType.GUILD_PUBLIC_THREAD;
			} else {
				if (ticket.type === ChannelType.GUILD_PUBLIC_THREAD) ticket.type = ChannelType.GUILD_PRIVATE_THREAD;
			}
		}
