...
import { Ban, Config, DiscordApiErrors, GuildBanAddEvent, GuildBanRemoveEvent, Member, User, emitEvent } from "@spacebar/util";
...
router.get(
	...
	async (req: Request, res: Response) => {
		...
		if (!query || query.trim().length === 0 || query.length > maxUsername) {
			throw new HTTPError(`The query must be between 1 and ${maxUsername} characters in length`);
		}
