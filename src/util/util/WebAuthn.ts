...
export async function generateWebAuthnTicket(challenge: string): Promise<string> {
	return new Promise((res, rej) => {
		jwt.sign({ challenge }, Config.get().security.jwtSecret, jwtSignOptions, (err, token) => {
...
export async function verifyWebAuthnToken(token: string) {
	return new Promise((res, rej) => {
		jwt.verify(token, Config.get().security.jwtSecret, jwtVerifyOptions, async (err, decoded) => {
