...
export async function onRequestGuildMembers(this: WebSocket, { d }: Payload) {
	...
	const maxGuildMembersLimit = Config.get().limits.guild.maxMembersRequestLimit || 100;
	if ((query || (user_ids && user_ids.length > 0)) && (!limit || limit > maxGuildMembersLimit)) limit = maxGuildMembersLimit;
